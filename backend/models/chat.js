...
function getChannels(req, res) {
    knex('users').select(['channels', 'pending_channels']).where({username: req.user}).then(allChannels => {
        ...
        if(allChannels[0]['pending_channels'].length != 0 || allChannels[0]['channels'].length != 0) {
            knex('channels').select('*').where(knex.raw("channelid in ('" + allChannels[0]['pending_channels'].join("','") + "')")).then(result => {
                ...
                res.json({channels: [...allChannels[0]['channels']], pending_channels: pending_channels});
            }, result => {
               console.log(result); 
            });
            
            knex('channels').select('*').where(knex.raw("channelid in ('" + allChannels[0]['channels'].join("','") + "')")).then(result => {
                let c = [];
                console.log(result);
                for(let i = 0; i < result.length; i++) {
                    let members = [...result[i]['members'], ...result[i]['pending_members']];
...
module.exports = {createChannel, getChannels, sendMessage, getMessages, joinChannel, leaveChannel};var knex = require('./database');
